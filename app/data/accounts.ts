/**
 * Demo account roles, keep in sync with backend/internal/chain/accounts.go
 * and backend/internal/chain/params.go (bond / seed balances).
 */
export const DEMO_ACCOUNTS = [
  { role: "Deployer", addr: "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266" },
  { role: "Sequencer", addr: "0x70997970C51812dc3A010C7d01b50e0d17dc79C8" },
  { role: "Challenger", addr: "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC" },
  { role: "Trader 0", addr: "0x90F79bf6EB2c4f870365E785982E1f101E93b906" },
  { role: "Trader 1", addr: "0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65" },
] as const;

/** Matches OptimisticPortalMock.BOND_AMOUNT (0.1 ETH). */
export const PORTAL_BOND_ETH = 0.1;

/** balanceA seeded per trader on swap engines. */
export const TRADER_SEED_BALANCE = 10_000;

/** Native ETH topped up on each demo account at session start (Anvil). */
export const DEMO_ACCOUNT_ETH = 10_000;
