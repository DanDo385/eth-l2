    ...
    <div className="space-y-1.5 border-t border-zinc-800/80 pt-2">
      ...
      {lying && (
        <div className="rounded border border-red-900/40 bg-red-950/15 px-2 py-1.5">
          <p className="font-mono text-[8px] text-red-400/90">
            lying · {lying.file}:{lying.line}
          </p>
          <pre className="mt-1 overflow-x-auto font-mono text-[9px] leading-relaxed text-red-200/90">
      ...
      {honest && (
        <div className="rounded border border-emerald-900/40 bg-emerald-950/15 px-2 py-1.5">
          <p className="font-mono text-[8px] text-emerald-400/90">
            honest · {honest.file}:{honest.line}
          </p>
          <pre className="mt-1 overflow-x-auto font-mono text-[9px] leading-relaxed text-emerald-200/90">
...
function EventDetail({
  ...
  return (
    <div className="space-y-1.5 border-t border-zinc-800/80 pt-2 text-[10px] leading-relaxed text-zinc-400">
      <div className="grid grid-cols-[5.5rem_1fr] gap-x-2 gap-y-1 font-mono">
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <span className="shrink-0 font-mono text-[9px] text-zinc-600">#{entry.seq}</span>
        <span
          className={`shrink-0 rounded border px-1 py-0.5 text-[8px] font-semibold uppercase tracking-wide ${layerClass(entry.layer)}`}
        >
          {entry.layer}
        </span>
        <div className="min-w-0 flex-1">
          ...
          {!expanded && (
            <p className="mt-0.5 truncate text-[10px] text-zinc-500">{entry.summary}</p>
          )}
        </div>
        <span
          aria-hidden="true"
  ...
  return (
    <div className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900 p-3 sm:p-4">
      ...
      <div className="space-y-1.5">
        ...
        {recent.length === 0 ? (
          <p className="text-[10px] text-zinc-600">No events yet — start a demo.</p>
        ) : (
          <div className="grid gap-1 sm:grid-cols-2 xl:grid-cols-5">
            {recent.map((entry, i) => (
              ...
              >
                <span
                  className={`shrink-0 rounded border px-1 py-0.5 text-[8px] font-semibold uppercase ${layerClass(entry.layer)}`}
                >
                  {entry.layer}
                </span>
                <p className="min-w-0 flex-1 truncate text-[10px] text-zinc-300">
                  {entry.summary}
                </p>
