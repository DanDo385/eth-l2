  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-1 mb-1">
        <span className="text-xs font-semibold text-zinc-200">Batch #{rollup.batchId}</span>
      ...
      <p className="text-xs leading-snug text-zinc-400 line-clamp-2">{status.explanation}</p>
      <p className="text-xs mt-1 font-mono text-zinc-500 truncate" title={rollup.claimedPostRoot}>
        claim {rollup.claimedPostRoot ? shortHash(rollup.claimedPostRoot) : "pending"}
      </p>
      {rollup.txCount != null && rollup.txCount > 0 && (
        <p className="text-xs text-zinc-600">{rollup.txCount} swap{rollup.txCount === 1 ? "" : "s"}</p>
      )}
    </motion.button>
  );
}
